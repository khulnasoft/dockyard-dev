import type { Container, ContainerMount, ContainerPort, ContainerStat } from "~/types/containers/dockyardContainers";
...
const getIconUrl = async (labels: Container['labels']) => {
    if (labels && labels['sh.dockyard.icon']) {
        return checkUrl(labels['sh.dockyard.icon']);
    }
    if (
        ...
        switch (vendor) {
            ...
            default: {
                return labels['sh.dockyard.icon']
            }
        }
    }
}
...
export const normalizeContainerInfo = async (data: FixedContainerInfo): Promise<Container> => {
    return {
        ...
        info: {
            title:
                data.Labels['sh.dockyard.title'] ||
                data.Labels['org.opencontainers.image.title'],
            notes: data.Labels['sh.dockyard.notes'],
            description: data.Labels['org.opencontainers.image.description'],
            external: data.Labels['sh.dockyard.external'],
            subdomain: data.Labels['sh.dockyard.subdomain'],
...
export const normalizeContainerInspectInfo = async (data: FixedContainerInspectInfo): Promise<Container> => {
    return {
        ...
        info: {
            title:
                data.Config.Labels['sh.dockyard.title'] ||
                data.Config.Labels['org.opencontainers.image.title'],
            notes: data.Config.Labels['sh.dockyard.notes'],
            description: data.Config.Labels['org.opencontainers.image.description'],
            external: data.Config.Labels['sh.dockyard.external'],
            subdomain: data.Config.Labels['sh.dockyard.subdomain'],
...
const transformInfo = async (data: CreateContainerForm) => {
    ...
    const [baseLabels, infoLabels, portLabels, envLabels] = await Promise.all([
        ...
        info ? Object.fromEntries(Object.entries(info).map(([key, value]) => [`sh.dockyard.${key}`, value])) : null,
        ports ? Object.fromEntries(ports.map((port) => [`sh.dockyard.${port.host}`, port.label])) : null,
        env ? Object.fromEntries(Object.entries(env).map(([name, env]) => [`sh.dockyard.env.${name}.label`, env.label, `sh.dockyard.env.${name}.description`, env.description])) : null
    ])
