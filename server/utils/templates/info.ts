import { dockyardTemplateSchema, type PortainerV1Template, type PortainerV2Template, type DockyardTemplate, type DockyardV1Template, type DockyardV2Template } from "~/types/templates/dockyard";
...
export interface addDockyardTemplate {
    url: string;
    name: string;
    title?: string;
}

export const getTemplates = async (): Promise<DockyardTemplate[]> => {
    const templates = [] as DockyardTemplate[]
    ...
    const teamplatePromises = templateList.map(async (template) => {
        ...
        const isValid = dockyardTemplateSchema.deepPartial().safeParse(_template).success
        if (_template && isValid) {
            templates.push(_template as DockyardTemplate)
        } else {
...
type TemplateVersions = PortainerV1Template[] | DockyardV1Template[] | DockyardTemplate | PortainerV2Template
...
export const addTemplate = async ({ url, name: _name, title: _title }: addDockyardTemplate) => {
