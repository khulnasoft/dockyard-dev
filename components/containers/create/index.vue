...
import { dockyardV2TemplatePortSchema, type DockyardV2TemplatePort, type DockyardTemplate, dockyardV1TemplatePortSchema, type DockyardV1TemplatePort, type dockyardV1TemplatePorts } from '~/types/templates/dockyard';
...
const template = defineModel<DockyardTemplate['templates'][0]>('template')
...
watch(dialog, (open) => {
  if (open) {
    ...
    localStorage.setItem('dockyard_savedContainerForm', JSON.stringify(values))
  }
})

const populateFromLocalStorage = () => {
  const savedContainerForm = localStorage.getItem('dockyard_savedContainerForm')
...
const populateFromTemplate = async (template: DockyardTemplate['templates'][0]): Promise<PartialDeep<CreateContainerForm>> => {
  return {
    name: template?.name || '',
...
</style>~/shared/templates/dockyard~/shared/containers/create