  ...
  <v-card-item class="d-flex flex-column items-center">
    ...
    >
      <v-row no-gutters>
        ...
        >
          <v-col v-if="labels && labels[`sh.dockyard.${port.containerPort}`]">
            <v-tooltip
              location="bottom"
            ...
            >
              <template #activator="{ props }">
                ...
                >
                  {{ labels[`sh.dockyard.${port.containerPort}`] }}
                </v-chip>
              </template>
            </v-tooltip>
          </v-col>
...
import type { ContainerPort, Container } from '~/types/containers/dockyardContainers';
