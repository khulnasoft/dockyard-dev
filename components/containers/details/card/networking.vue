                ...
                <v-tooltip :text="port.hostPort.toString()">
                  <template v-slot:activator="{ props }">
                    <v-btn v-bind="props" prepend-icon="mdi-link-variant" color="primary">{{
                      container.labels && container.labels[`sh.dockyard.${port.hostPort}`] ||
                      port.hostPort }}</v-btn>
                  </template>
                </v-tooltip>
              </td>
...
import type { Container } from '~/types/containers/dockyardContainers';
...
defineProps<Props>()
const tab = ref(0)
</script>~/shared/containers/dockyardContainers