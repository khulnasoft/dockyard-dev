        ...
        <v-fade-transition>
          <div v-if="search.length < 1 && template.featured">
            <lazy-templates-list-ecarousel
              :template="template"
              @create-app="(app: DockyardTemplate['templates'][0]) => createContainerFromTemplate(app)"
            />
...
const selectedApp = ref<DockyardTemplate["templates"][0] | undefined>();
const openInfo = ref(false);
const maximize = ref(false);

const createContainerFromTemplate = (app: DockyardTemplate["templates"][0]) => {
