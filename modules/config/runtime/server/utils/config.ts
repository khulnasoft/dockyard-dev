...
import { defaultDockyardConfig } from "./defaults"
import fs from 'fs-extra'
import { stringifyYAML } from 'confbox'
import { resolve } from 'path'
import { DockyardConfigSchema, type DockyardConfig } from "../../../types"
...
const DefaultWatchOptions = {
    cwd: nuxtConfig.dockyard.configOptions.configPath,
    configFile: 'config.yaml',
    chokidarOptions: { ignoreInitial: false },
    defaults: defaultDockyardConfig
}
...
export const configPaths = {
    secrets: join(nuxtConfig.dockyard.configOptions.configPath, '.secrets.json'),
    ssh: join(nuxtConfig.dockyard.configOptions.configPath, '.ssh/'),
    auth: join(nuxtConfig.dockyard.configOptions.configPath, '.auth/'),
    backups: {
        config: join(nuxtConfig.dockyard.configOptions.configPath, 'backups/config/'),
        instance: join(nuxtConfig.dockyard.configOptions.configPath, 'backups/instance/')
    },
    templates: join(nuxtConfig.dockyard.configOptions.configPath, 'templates/')
}
...
export const useConfig = async (): Promise<DockyardConfig> => {
...
export const updateConfig = async (config: Omit<DockyardConfig, 'secrets'>, path: string) => {
    ...
    try {
        ...
        DockyardConfigSchema.parse(config)
...
export const checkConfig = async () => {
    ...
    if (!configExists) {
        ...
        updateConfig(defaultDockyardConfig, config.cwd)
    } else {
        try {
            ...
            DockyardConfigSchema.parse(config.config)
